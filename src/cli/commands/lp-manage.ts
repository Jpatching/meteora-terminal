...
export function registerLpCommand(program: Command) {
  const lp = program
    .command("lp")
    .description("Manage DLMM Liquidity Positions");
  ...
  lp.command("open")
    ...
    .option("--min <num>", "Min of range", `0.99`)
    .option("--max <num>", "Max of range", `1.01`)
    .action(
      async (opts: {
        pair?: string;
        pool?: string;
        ...
        if (!opts.pool && !opts.pair) {
          throw new Error(
            "Provide either --pool <address> or --pair <SYMA-SYMB>.",
          );
        }
        ...
        if (!Number.isFinite(min) || !Number.isFinite(max)) {
          throw new Error(
            "Provide --range <min-max> (e.g. 0.98-1.02) OR both --min and --max.",
          );
        }
  ...
  lp.command("resolve")
    ...
    .action(async (opts: { pair: string }) => {
      ...
      console.log(
        `pair=${opts.pair} -> name="${name}" address=${address.toBase58()}`,
      );
    });
}
